...
namespace TTG_Tools
{
    public partial class ArchivePacker : Form
    {
        ...
        {
            if (messageListBox == null) { Console.WriteLine(report); return; }
        ...
        public static void PackTtarchCli(string inputFolder, string outputPath, byte[] key, int versionArchive, bool compression, bool encryptCheck, bool dontEncLua, int compressAlgorithm)
        {
            var inst = (ArchivePacker)System.Runtime.Serialization.FormatterServices.GetUninitializedObject(typeof(ArchivePacker));
            var m = typeof(ArchivePacker).GetMethod("ttarchBuilder", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            var task = (System.Threading.Tasks.Task)m.Invoke(inst, new object[] { inputFolder, outputPath, key, compression, versionArchive, encryptCheck, dontEncLua, compressAlgorithm });
            task.Wait();
        }

        private static byte[] ZlibCompressor(byte[] bytes) //Для старых архивов (с версии 3 по 7)
        {
            // The zlib stream must be disposed before reading the output, otherwise the
            // trailing Adler32 + Z_STREAM_END marker isn't flushed and the resulting bytes
            // are an incomplete zlib stream. Strict decompressors (and at least some game
            // engines, e.g. Wallace & Gromit Ep1) reject the chunk and crash.
        ...
        async Task ttarch2Builder(string inputFolder, string outputPath, bool compression, bool encryption, bool encLua, byte[] key, int versionArchive, bool newEngine, int compressAlgorithm)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                using (BinaryWriter mbw = new BinaryWriter(ms))
                {
                    for (int i = 0; i < fi.Length; i++)
                    {
                        ...
                        mbw.Write(zeros);
                        mbw.Write((ushort)(currentNameOff / 0x10000)); //Name table block
                        mbw.Write((ushort)(currentNameOff % 0x10000)); //Name offset inside block
            ...
            tmp = BitConverter.GetBytes((uint)nameSize);
            Array.Copy(tmp, 0, chunk, chunkOff, tmp.Length);
            chunkOff += (uint)tmp.Length;
        ...
        async Task ttarchBuilder(string inputFolder, string outputPath, byte[] key, bool compression, int versionArchive, bool encryptCheck, bool DontEncLua, int compressAlgorithm) //Функция сборки
        {
            ...
            // ttarchext reference always writes 0 directories: TTARCH file names are
            // stored flat (no folder prefix), so the "folders" list is metadata only and
            // the game engine doesn't need it to locate files. The previous code wrote a
            // bogus directory entry containing the user's full local Windows path when the
            // input folder had no subdirectories, which crashes games like Wallace & Gromit
            // that actually iterate this list.
            int directories = 0;
            ...
            // Original v4 archives (verified against Wallace & Gromit Ep.1 PC) end the
            // file table immediately after the last file entry's size. The 5 stray bytes
            // (int + byte) previously appended here matched neither the original format
            // nor the ttarchext reference, and caused the game's header parser to crash.
            ...
            while(ch < chunksCount && a < fi.Length)
            {
                ...
                // Don't re-encrypt files that are already encrypted. The TTG Tools unpacker
                // keeps .lenc files as-is (encrypted) by default; the previous code blindly
                // re-encrypted them on repack, producing doubly-encrypted Lua that crashes
                // games on load (verified against Wallace & Gromit Ep.1 PC).
                if (!DontEncLua && ((fi[a].Extension.ToLower() == ".lua") || (fi[a].Extension.ToLower() == ".lenc"))
                    && !Methods.isLuaEncrypted(file))
                {
                    file = Methods.encryptLua(file, key, false, versionArchive);
                }
