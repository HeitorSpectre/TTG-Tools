...
namespace TTG_Tools
{
    class Methods
    {
        ...
        #region Import text normalization (CJK)
        //Applies the optional text-normalization passes to a translated string during import.
        //All passes are opt-in through MainMenu.settings and only touch text outside Telltale
        //control markers ([...] and {...}). Mostly useful for Chinese/Japanese/Korean localizations.
        public static string NormalizeImportedText(string text)
        {
            if (string.IsNullOrEmpty(text) || MainMenu.settings == null) return text;

            string result = text;

            if (MainMenu.settings.normalizePunctuationBeforeNewlineInImport)
            {
                //Keep the line break after a sentence-ending mark instead of before it,
                //preventing malformed sequences like "\n。" -> "。\n".
                result = TransformOutsideMarkers(result, NormalizePunctuationBeforeNewline);
            }

            //The remaining passes only make sense when there is actual CJK text.
        ...
        //Runs the given transform only on the parts of the text that are not Telltale control
        //markers, so tags such as [pause] or {icon} are never altered.
        private static string TransformOutsideMarkers(string text, Func<string, string> transformer)
        {
            ...
            for (int i = 0; i < segments.Length; i++)
            {
                ...
                if (IsMarkerSegment(segment)) sb.Append(segment);
                else sb.Append(transformer(segment));
            }
        ...
        private static string NormalizePunctuationBeforeNewline(string text)
        {
            ...
            //Move a CJK sentence-ending mark that ended up right after a line break back
            //in front of it. Only matches this specific malformed case, so ordinary line
        ...
        //Drops whitespace that sits between two CJK characters (common artifact of
        //machine translation / copy-paste), keeping the natural CJK text flow.
        ...
        //Converts an ASCII dot adjacent to CJK text into a full-width Chinese period (。).
        ...
        private static readonly KeyValuePair<string, string>[] CsiPs2ControllerTags =
        {
            new KeyValuePair<string, string>("\u201E", "[Cross]"),
            new KeyValuePair<string, string>("\u20AC", "[Square]"),
            new KeyValuePair<string, string>("\u0192", "[Triangle]"),
            new KeyValuePair<string, string>("\u201A", "[Circle]"),
            new KeyValuePair<string, string>("\u2021", "[D-Pad Left]"),
            new KeyValuePair<string, string>("\u02C6", "[D-Pad Down]"),
            new KeyValuePair<string, string>("\u2030", "[D-Pad Right]"),
            new KeyValuePair<string, string>("\u0160", "[D-Pad Up]"),
            new KeyValuePair<string, string>("\u008D", "[L1]"),
            new KeyValuePair<string, string>("\u0161", "[R1]"),
            new KeyValuePair<string, string>("\u2020", "[Left Analog Stick]"),
            new KeyValuePair<string, string>("\u2039", "[Select]"),
            new KeyValuePair<string, string>("\u0152", "[Start]"),
            new KeyValuePair<string, string>("\u203A", "[L2]"),
            new KeyValuePair<string, string>("\u0153", "[R2]"),
            new KeyValuePair<string, string>("\u00A5", "[Right Analog Stick]")
        };

        public static string DecodeCsiPs2ControllerTags(string text)
        {
            if (String.IsNullOrEmpty(text)) return text;

            for (int i = 0; i < CsiPs2ControllerTags.Length; i++)
            {
                text = text.Replace(CsiPs2ControllerTags[i].Key, CsiPs2ControllerTags[i].Value);
            }

            return text;
        }

        public static string EncodeCsiPs2ControllerTags(string text)
        {
            if (String.IsNullOrEmpty(text)) return text;

            for (int i = 0; i < CsiPs2ControllerTags.Length; i++)
            {
                text = text.Replace(CsiPs2ControllerTags[i].Value, CsiPs2ControllerTags[i].Key);
            }
        ...
        public static string DecodeGameText(byte[] bytes, bool useUtf8)
        {
            ...
            if (useUtf8)
            {
                decoded = decodedUtf8 ?? Encoding.GetEncoding(codePage).GetString(bytes);
                return MainMenu.settings.swizzlePS2 ? DecodeCsiPs2ControllerTags(decoded) : decoded;
            }
        ...
        {
            if (MainMenu.settings.swizzlePS2)
            {
                text = EncodeCsiPs2ControllerTags(text);
            }
